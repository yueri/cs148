...
std::shared_ptr<Camera> Assignment7::CreateCamera() const
{
    ...
    // front view
//    camera->SetPosition(glm::vec3(0.f, 10.0f, 20.f));
...
std::shared_ptr<Scene> Assignment7::CreateScene() const
{
    ...
    std::vector<std::shared_ptr<aiMaterial>> loadedMaterials9;
    std::vector<std::shared_ptr<MeshObject>> mirrorObjects = MeshLoader::LoadMesh("mirror/mirror.obj", &loadedMaterials9);
    for (size_t i = 0; i < mirrorObjects.size(); ++i) {
        std::shared_ptr<Material> materialCopy = cubeMaterial->Clone();
        materialCopy->LoadMaterialFromAssimp(loadedMaterials9[i]);
        materialCopy->SetReflectivity(1.f);
        mirrorObjects[i]->SetMaterial(materialCopy);

        std::shared_ptr<SceneObject> mirrorSceneObject = std::make_shared<SceneObject>();
        mirrorSceneObject->AddMeshObject(mirrorObjects[i]);
        mirrorSceneObject->SetPosition(glm::vec3(0.0f, 0.f, 0.f));
        // cubeSceneObject->Rotate(glm::vec3(1.f, 0.f, 0.f), PI / 2.f);

        mirrorSceneObject->CreateAccelerationData(AccelerationTypes::BVH);
        mirrorSceneObject->ConfigureAccelerationStructure([](AccelerationStructure* genericAccelerator) {
            BVHAcceleration* accelerator = dynamic_cast<BVHAcceleration*>(genericAccelerator);
            accelerator->SetMaximumChildren(2);
            accelerator->SetNodesOnLeaves(2);
        });

        mirrorSceneObject->ConfigureChildMeshAccelerationStructure([](AccelerationStructure* genericAccelerator) {
            BVHAcceleration* accelerator = dynamic_cast<BVHAcceleration*>(genericAccelerator);
            accelerator->SetMaximumChildren(2);
            accelerator->SetNodesOnLeaves(2);
        });
        newScene->AddSceneObject(mirrorSceneObject);
    }

    // Lights
    std::shared_ptr<Light> pointLight = std::make_shared<PointLight>();
    pointLight->SetPosition(glm::vec3(15.0f, 20.0f, 15.0f));
